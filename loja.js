...
(function () {
  ...
  function renderPayArea() {
    ...
    if (state.method === "card") {
      a.innerHTML =
        `<div class="field" style="margin-top:14px"><label>Número do cartão</label><input placeholder="0000 0000 0000 0000" disabled></div>` +
        `<div class="f2"><div class="field"><label>Validade</label><input placeholder="MM/AA" disabled></div>` +
        `<div class="field"><label>CVV</label><input placeholder="000" disabled></div></div>`;
    } else {
      a.innerHTML = `<div style="text-align:center"><div class="qr"></div><div style="font-size:13px;color:var(--muted)">Escaneie para pagar (demonstração)</div></div>`;
    }
  }
  ...
  async function confirmAd() {
    ...
    try {
      const { data, error } = await sb.rpc("criar_adesao", {
        p_plano_slug: state.plan, p_titular: titular, p_dependentes: dependentes,
        p_ciclo: state.bill === "ano" ? "anual" : "mensal",
        p_forma_pagamento: state.method === "pix" ? "pix" : "cartao", p_cartao_final: null,
      });
      if (error) throw error;
      state.ultimaAdesao = data;
      const titularCard = (data.cartoes || []).find((c) => c.papel === "Titular") || data.cartoes?.[0];
      const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
      set("c_nome", (titular.nome || "Titular").toUpperCase());
      set("c_plan", data.plano);
      set("c_num", titularCard ? titularCard.numero : data.numero_conta + "-01");
      const tot = data.ciclo === "anual" ? money(data.valor_fatura) + " à vista" : money(data.valor_fatura) + "/mês";
      set("doneMsg", `Plano ${data.plano} · ${tot} · ${dependentes.length} dependente(s). Conta ${data.numero_conta}. (Adesão registrada — pagamento em modo demonstração.)`);
      go("done");
    } catch (e) {
      console.error("Erro checkout:", e);
      alert("Não foi possível concluir a adesão: " + (e.message || e));
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = "Confirmar adesão (simulação) →"; }
    }
  }
